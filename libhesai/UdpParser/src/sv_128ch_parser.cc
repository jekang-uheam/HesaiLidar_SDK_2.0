...
SV128chParser<T_Point>::SV128chParser()
    : Udp1_4Parser<T_Point>() {
}
template <typename T_Point>
SV128chParser<T_Point>::~SV128chParser() {
}
template <typename T_Point>
int SV128chParser<T_Point>::DecodePacket(LidarDecodedFrame<T_Point> &frame, const UdpPacket &udpPacket) {
...
template <typename T_Point>
int SV128chParser<T_Point>::ComputeXYZI(LidarDecodedFrame<T_Point> &frame, int packet_index) {
...
template <typename T_Point>
void SV128chParser<T_Point>::LoadFiretimesFile(std::string firetimes_path) {
  try {
...
template <typename T_Point>
double SV128chParser<T_Point>::GetFiretimesCorrection(int laserId, double speed, uint8_t optMode, uint8_t angleState, float dist) {
  int idx = 0;
  switch (optMode) {
    case 0:
      idx = angleState;
...
template <typename T_Point>
bool SV128chParser<T_Point>::IsNeedFrameSplit(uint16_t azimuth) {
  ...
  uint16_t frame_start_azimuth = static_cast<uint16_t>(this->frame_start_azimuth_ * kResolutionInt);
  if (this->rotation_flag == 1) {
    // When an angle jump occurs
    if (this->last_azimuth_ - azimuth > division) {
      if (frame_start_azimuth > this->last_azimuth_ || frame_start_azimuth <= azimuth) {
        return true;
      }
      return false;
    }
    if (this->last_azimuth_ < azimuth && this->last_azimuth_ < frame_start_azimuth && azimuth >= frame_start_azimuth) {
      return true;
    }
    return false;
  } else {
